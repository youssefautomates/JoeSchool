...
export function ReviewModerationTable({
  ...
  const getProductName = (productId: string) => productNames[productId] || "غير معروف";
  ...
  const handleBulkDelete = () => {
    if (selectedIds.length === 0) return;
    const reason = prompt("يرجى كتابة سبب الأرشفة الجماعية (اختياري):") || undefined;
    if (window.confirm(`هل أنت متأكد من أرشفة ${selectedIds.length} تقييم؟`)) {
  ...
  const handleBulkRestore = () => {
    if (selectedIds.length === 0) return;
    if (window.confirm(`هل أنت متأكد من استعادة ${selectedIds.length} تقييم من الأرشيف؟`)) {
  ...
  const getStatusBadge = (status?: string) => {
    switch (status) {
      case "visible":
        return <span className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-2.5 py-1 rounded-lg text-[10px] font-bold font-cairo shrink-0">🟢 نشط</span>;
      case "hidden":
        return <span className="bg-red-500/10 text-red-400 border border-red-500/20 px-2.5 py-1 rounded-lg text-[10px] font-bold font-cairo shrink-0">🔴 مخفي</span>;
      case "pending":
        return <span className="bg-amber-500/10 text-amber-400 border border-amber-500/20 px-2.5 py-1 rounded-lg text-[10px] font-bold font-cairo shrink-0">⏳ معلق</span>;
      case "archived":
        return <span className="bg-zinc-800 text-zinc-500 border border-zinc-700/50 px-2.5 py-1 rounded-lg text-[10px] font-bold font-cairo shrink-0">📁 مؤرشف</span>;
      default:
        return <span className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-2.5 py-1 rounded-lg text-[10px] font-bold font-cairo shrink-0">🟢 نشط</span>;
    }
  };
  ...
  return (
    <div className="space-y-6 relative" dir="rtl">
      ...
      {selectedIds.length > 0 && (
        <div className="bg-rose-950/20 border border-rose-500/20 p-4 rounded-2xl flex items-center justify-between gap-4 flex-wrap text-xs font-bold font-cairo animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="flex items-center gap-2">
            <span className="text-white">تم تحديد <span className="text-rose-500 font-mono text-sm">{selectedIds.length}</span> تقييم في الصفحة الحالية.</span>
            {!selectAllAcross && selectedIds.length < reviews.length && (
              ...
              >
                تحديد كافة الـ {reviews.length} تقييم المطابقة للفلاتر؟
              </button>
            )}
            {selectAllAcross && (
              <span className="text-emerald-400 font-bold">✨ تم تحديد كافة التقييمات المطابقة للبحث عبر جميع الصفحات.</span>
            )}
          </div>
          <button 
      ...
      <div className="bg-[#09090e]/60 border border-white/5 rounded-2xl overflow-hidden backdrop-blur-xl">
        <div className="overflow-x-auto">
          <table className="w-full text-right border-collapse">
            <thead>
              <tr className="border-b border-white/5 bg-white/[0.01] text-zinc-400 font-bold text-xs font-cairo">
                ...
                <th className="p-4 font-alexandria">صاحب التقييم</th>
                <th className="p-4 font-alexandria">التقييم بالنجوم</th>
                <th className="p-4 font-alexandria max-w-xs">الشهادة / نص الرأي</th>
                <th className="p-4 font-alexandria">العنصر المرتبط</th>
                <th className="p-4 font-alexandria">تاريخ النشر</th>
                <th className="p-4 font-alexandria">الحالة</th>
                <th className="p-4 font-alexandria text-left">أدوات التحكم</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/5 text-xs text-zinc-300">
              {paginatedReviews.length === 0 ? (
                <tr>
                  <td colSpan={8} className="p-20 text-center text-zinc-500 font-bold font-cairo">
                ...
                paginatedReviews.map(r => (
                  ...
                  >
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        <div>
                          ...
                          <span className="text-[9px] text-zinc-500 font-bold font-cairo">
                            {r.source === "manual_admin" ? "إضافة يدوية" : r.source === "imported" ? "مستورد" : "مضاف تلقائيًا"}
                          </span>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="p-4 max-w-xs">
                      <p className="line-clamp-2 text-zinc-400 font-cairo italic leading-relaxed" title={r.text}>
                        &ldquo;{r.text}&rdquo;
                      </p>
                      {r.status === "archived" && r.archiveReason && (
                        <p className="text-[10px] text-red-400 font-cairo mt-1 block">
                          سبب الأرشفة: {r.archiveReason}
                        </p>
                      )}
                    </td>
                    ...
                    <td className="p-4 text-left">
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        <button
                          ...
                          title={r.status === "archived" ? "حذف نهائي" : "أرشفة التقييم"}
                        >
      ...
      {/* Pagination Controls */}
      {totalReviewsCount > 0 && (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 font-cairo pt-2 select-none">
          <div className="flex items-center gap-4 flex-wrap">
            <span className="text-[10px] text-zinc-500 font-bold">
              الصفحة {activePage} من أصل {activeTotalPages} (يعرض {paginatedReviews.length} من {totalReviewsCount} تقييم)
            </span>
            
            {/* Page Size Selector */}
            <div className="flex items-center gap-1.5">
              <span className="text-[10px] text-zinc-500 font-bold">حجم الصفحة:</span>
              <select
      ...
      {selectedIds.length > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-[#09090f]/95 border border-rose-500/25 p-4 md:p-5 rounded-2xl flex items-center justify-between gap-6 shadow-[0_15px_40px_rgba(214,0,75,0.2)] backdrop-blur-2xl max-w-xl w-[90%] z-40 animate-in fade-in slide-in-from-bottom-6 duration-300 font-cairo">
          <div className="flex flex-col">
            <span className="text-white text-xs font-black">إدارة جماعية للتقييمات</span>
            <span className="text-[10px] text-zinc-400 mt-0.5">تم اختيار <span className="text-rose-500 font-mono font-black text-xs">{selectedIds.length}</span> تقييم.</span>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap">
            ...
            >
              <Trash2 className="w-3.5 h-3.5" />
              <span>{hasArchivedSelected ? "حذف نهائي" : "أرشفة"}</span>
            </button>
          </div>
        </div>
      )}
