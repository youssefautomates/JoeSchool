...
export default function ReviewsPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const combined: DisplayProduct[] = [
        ...(productsData || []).map(p => ({ id: p.id, title: `🛍️ [Product] - ${p.title}` })),
        ...(coursesData || []).map(c => ({ id: c.id, title: `🎓 [Course] - ${c.title}` })),
        ...(bundlesData || []).map(b => ({ id: b.id, title: `📦 [Bundle] - ${b.title}` }))
      ];
      ...
      toast.error("Failed to retrieve reviews data");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    if (!newReview.productId || !newReview.firstName || !newReview.text) {
      toast.error("Please fill in all required fields (Product, Name, Testimonial)");
      return;
    }
  ...
  const getProductName = (id: string) => products.find(p => p.id === id)?.title || "Unknown Item";
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 p-2 md:p-8 font-sans">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-white/5">
        <div>
          <h1 className="text-2xl font-black text-white mb-2">Manage Student Reviews</h1>
          <p className="text-zinc-500 text-xs">Full control over customer testimonials displayed across product detail pages.</p>
        </div>
        <button
          onClick={() => setIsAdding(!isAdding)}
          className="h-10 px-6 bg-[#D6004B] hover:bg-[#ff0059] text-white text-xs font-bold rounded-xl flex items-center gap-2 transition-all"
        >
      ...
      <AnimatePresence>
        {isAdding && (
          ...
          >
            <div className="bg-[#09090e] p-8 rounded-2xl border border-white/5 mb-8">
              <h3 className="text-sm font-bold uppercase tracking-wider text-white mb-6">Create New Customer Testimonial</h3>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div className="space-y-3 md:col-span-2">
                  <Label className="text-zinc-300 font-bold">Linked Digital Product or Course</Label>
                  <select 
                    value={newReview.productId}
                    onChange={(e) => setNewReview({...newReview, productId: e.target.value})}
                    className="w-full h-11 px-4 rounded-xl bg-white/5 border border-white/10 text-white font-sans text-xs outline-none focus:border-rose-500 cursor-pointer"
                  >
                    <option value="" disabled className="bg-zinc-900 text-zinc-400">-- Select a Product --</option>
                    {products.map(p => (
                      <option key={p.id} value={p.id} className="bg-[#09090e] text-white">{p.title}</option>
                    ))}
                  </select>
                </div>

                <div className="space-y-3">
                  <Label className="text-zinc-300 font-bold text-xs">First Name (Displayed fully)</Label>
                  <Input 
                ...
                <div className="space-y-3">
                  <Label className="text-zinc-300 font-bold text-xs">Last Name (First character displayed only)</Label>
                  <Input 
                ...
                <div className="space-y-3 md:col-span-2">
                  <Label className="text-zinc-300 font-bold text-xs">Review Score (Star Rating)</Label>
                  <div className="flex gap-2 bg-white/5 p-3 rounded-xl border border-white/10 w-fit cursor-pointer">
                    {[1,2,3,4,5].map(star => (
                      <Star 
                        key={star} 
                        onClick={() => setNewReview({...newReview, rating: star})}
                        className={`w-5 h-5 transition-colors ${newReview.rating >= star ? 'text-yellow-400 fill-current' : 'text-zinc-600'}`} 
                      />
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-3 md:col-span-2">
                  <div className="flex justify-between items-center">
                    <Label className="text-zinc-300 font-bold text-xs">Select Reviewer Cartoon Avatar</Label>
                    <span className="text-[10px] text-zinc-500 font-bold uppercase tracking-wider">
                      {newReview.gender === "male" ? "Male Avatar 🧔" : "Female Avatar 👩"}
                    </span>
                  </div>
                ...
                <div className="space-y-3 md:col-span-2">
                  <Label className="text-zinc-300 font-bold text-xs">Custom Image URL (Optional - overrides selected avatar)</Label>
                  <Input 
                    ...
                    className="h-11 bg-white/5 border-white/10 text-white dir-ltr text-left text-xs"
                  />
                </div>

                <div className="space-y-3 md:col-span-2">
                  <Label className="text-zinc-300 font-bold text-xs">Review Testimonial Text</Label>
                  <textarea 
                    value={newReview.text}
                    onChange={(e) => setNewReview({...newReview, text: e.target.value})}
                    placeholder="Write what the customer stated..."
                    className="w-full h-32 p-4 rounded-xl bg-white/5 border border-white/10 text-white font-sans text-xs outline-none focus:border-rose-500 resize-none custom-scrollbar"
                  />
                </div>
                ...
                <div className="space-y-4 md:col-span-2 bg-white/5 p-4 rounded-xl border border-white/5">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <Label className="text-zinc-300 font-bold text-xs">Verified Purchaser Badge</Label>
                    </div>
                    <Switch 
              ...
              <div className="flex justify-end gap-4 mt-8 pt-6 border-t border-white/10">
                <button
                  ...
                  className="px-6 py-2 text-zinc-500 text-xs font-bold hover:text-white transition-colors"
                >
                  Cancel
                </button>
                <button
                  onClick={handleSave}
                  disabled={saving}
                  className="px-8 py-2 bg-[#D6004B] hover:bg-[#ff0059] text-white text-xs font-bold rounded-xl flex items-center gap-2 transition-all disabled:opacity-50"
                >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {reviews.length === 0 && !isAdding && (
          <div className="col-span-full flex flex-col items-center justify-center py-20 bg-white/5 rounded-3xl border border-white/5 border-dashed">
            <MessageSquareQuote className="w-12 h-12 text-zinc-700 mb-4" />
            <h3 className="text-lg font-bold text-white mb-2">No reviews found yet</h3>
            <p className="text-zinc-500 text-xs font-semibold">Start by adding your first review to build trust and increase sales.</p>
          </div>
        )}

        {reviews.map(review => (
          <div key={review.id} className={`bg-[#09090e] p-6 rounded-2xl border transition-all ${review.isHidden ? 'border-red-500/30 opacity-50' : 'border-white/5 hover:border-rose-500/30'} flex flex-col`}>
            <div className="flex items-start justify-between mb-4">
                ...
                <div>
                  ...
                  {review.isVerified && (
                    <div className="flex items-center gap-1 mt-0.5">
                      ...
                      <span className="text-[9px] text-emerald-400 font-bold uppercase tracking-wider">Verified Purchaser</span>
                    </div>
                  )}
                </div>
              </div>
              <div className="flex gap-0.5 text-yellow-400">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star key={i} className={`w-3 h-3 ${i < review.rating ? 'fill-current' : 'text-zinc-700 fill-transparent'}`} />
                ))}
              </div>
            </div>

            <p className="text-zinc-400 text-xs leading-relaxed mb-6 flex-1 italic">"{review.text}"</p>

            <div className="pt-4 border-t border-white/5 space-y-4">
              <div className="flex items-center justify-between text-[10px] text-zinc-500 font-bold">
                <span>Product:</span>
                <span className="text-zinc-300 font-bold truncate max-w-[170px]">{getProductName(review.productId)}</span>
              </div>
              <div className="flex items-center justify-between gap-2">
                <button
                  onClick={() => handleToggleHide(review)}
                  className={`flex-1 py-2 rounded-lg text-xs font-bold transition-colors ${review.isHidden ? 'bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20' : 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700 hover:text-white'}`}
                >
