  ...
  return (
    <div className="space-y-8 font-sans text-zinc-900 min-h-screen pb-16 text-left animate-in fade-in duration-700" dir="ltr">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <Link
          href="/admin"
          className="inline-flex items-center gap-2 text-zinc-500 hover:text-zinc-900 text-xs font-bold transition-all"
        >
        ...
        <div className="flex items-center gap-3">
            ...
            {[
              ...
              <button
                ...
                className={`px-4 py-1.5 rounded-2xl text-xs font-bold transition-all ${
                  dateRange === range.value ? "bg-[#1D4ED8] text-white shadow-sm border border-zinc-200/60 shadow-[#1D4ED8]/20" : "text-zinc-500 hover:text-white"
                }`}
              >
                {range.label}
              </button>
          ...
          <button
            onClick={loadData}
            disabled={loading}
            className="flex items-center justify-center w-11 h-11 rounded-2xl transition-all bg-zinc-100/40 border border-zinc-200/60 hover:bg-zinc-100/80 hover:border-zinc-200 text-zinc-700 hover:text-zinc-900 cursor-pointer disabled:opacity-50"
          >
      ...
      {/* Title & Headline */}
      <div className="pb-6 border-b border-zinc-200/60 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight bg-gradient-to-r from-white via-zinc-200 to-brand-500 bg-clip-text text-transparent flex items-center gap-3">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        <div className="bg-slate-50/60 border border-zinc-200/60 p-5 rounded-2xl flex flex-col justify-between">
          <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest block mb-2">Total Revenue</span>
          <span className="text-lg font-black text-zinc-900">{formatPrice(summaryStats.totalRevenue, "EGP")}</span>
        ...
        <div className="bg-slate-50/60 border border-zinc-200/60 p-5 rounded-2xl flex flex-col justify-between">
          ...
          <span className="text-lg font-black text-zinc-800">{summaryStats.totalViews.toLocaleString()}</span>
        ...
        <div className="bg-slate-50/60 border border-zinc-200/60 p-5 rounded-2xl flex flex-col justify-between">
          ...
          <span className="text-lg font-black text-yellow-500">{summaryStats.totalCheckoutStarteds.toLocaleString()}</span>
        ...
        <div className="bg-slate-50/60 border border-zinc-200/60 p-5 rounded-2xl flex flex-col justify-between">
          ...
          <span className="text-lg font-black text-zinc-900">{summaryStats.totalPurchases.toLocaleString()}</span>
          <span className="text-[10px] text-zinc-500 mt-2">Orders successfully placed</span>
        </div>
      </div>

      {/* Filter and Search Bar Row */}
      <div className="flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center bg-slate-50/40 border border-zinc-200/60 p-4 rounded-3xl">
        <div className="relative w-full lg:max-w-md group">
          <Search className="w-5 h-5 absolute left-4 top-1/2 -translate-y-1/2 text-zinc-500 group-focus-within:text-[#1D4ED8] transition-colors" />
          <input 
            type="text" 
            placeholder="Search by course or product name..." 
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full bg-zinc-100/40 border border-zinc-200/60 rounded-2xl py-3.5 pl-12 pr-4 text-xs font-sans focus:outline-none focus:border-[#1D4ED8]/50 focus:bg-zinc-100/80 transition-all text-zinc-800"
          />
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4 w-full lg:w-auto">
          ...
          <div className="flex items-center bg-zinc-100/40 border border-zinc-200/60 rounded-2xl p-1 gap-1">
            {[
              ...
              <button
                ...
                className={`px-4 py-2 rounded-2xl text-[11px] font-bold transition-all ${
                  typeFilter === type.value ? "bg-zinc-100/80 text-zinc-900" : "text-zinc-500 hover:text-zinc-900"
                }`}
              >
                {type.label}
              </button>
          ...
          <button
            ...
            className={`flex items-center gap-2.5 px-5 h-11 rounded-2xl text-xs font-bold border transition-all ${
              activeOnly
                ? "bg-[#1D4ED8]/10 border-[#1D4ED8]/30 hover:bg-[#1D4ED8] text-[#1D4ED8] hover:text-white"
                : "bg-zinc-100/40 border-zinc-200/60 text-zinc-500 hover:text-white hover:bg-zinc-100/80"
            }`}
          >
      ...
      <div className="rounded-3xl bg-slate-50/60 border border-zinc-200/60 p-6 shadow-sm border border-zinc-200/60">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-zinc-200/60 text-zinc-500 text-xs select-none">
                <th className="pb-4 font-semibold text-left cursor-pointer hover:text-zinc-900 transition-colors" onClick={() => handleSort("title")}>
                ...
                <th className="pb-4 font-semibold text-center cursor-pointer hover:text-zinc-900 transition-colors" onClick={() => handleSort("views")}>
                  <div className="flex items-center justify-center gap-1.5">
                    Page Views
                    <ArrowUpDown className="w-3 h-3 opacity-60" />
                  </div>
                </th>
                <th className="pb-4 font-semibold text-center cursor-pointer hover:text-zinc-900 transition-colors" onClick={() => handleSort("checkoutPageOpeneds")}>
                  <div className="flex items-center justify-center gap-1.5">
                    Checkout Opened
                    <ArrowUpDown className="w-3 h-3 opacity-60" />
                  </div>
                </th>
                <th className="pb-4 font-semibold text-center cursor-pointer hover:text-zinc-900 transition-colors" onClick={() => handleSort("checkoutStarteds")}>
                  <div className="flex items-center justify-center gap-1.5">
                    Checkout Started
                    <ArrowUpDown className="w-3 h-3 opacity-60" />
                  </div>
                </th>
                <th className="pb-4 font-semibold text-center cursor-pointer hover:text-zinc-900 transition-colors" onClick={() => handleSort("purchases")}>
                  <div className="flex items-center justify-center gap-1.5">
                    Purchases
                    <ArrowUpDown className="w-3 h-3 opacity-60" />
                  </div>
                </th>
                <th className="pb-4 font-semibold text-center cursor-pointer hover:text-zinc-900 transition-colors" onClick={() => handleSort("revenue")}>
                  <div className="flex items-center justify-center gap-1.5">
                    Revenue
                    <ArrowUpDown className="w-3 h-3 opacity-60" />
                  </div>
                </th>
                <th className="pb-4 font-semibold text-center cursor-pointer hover:text-zinc-900 transition-colors" onClick={() => handleSort("conversionRate")}>
                  <div className="flex items-center justify-center gap-1.5">
                    Conversion Rate
                    <ArrowUpDown className="w-3 h-3 opacity-60" />
                  </div>
              ...
              {loading ? (
                Array.from({ length: 5 }).map((_, i) => (
                  <tr key={i} className="animate-pulse">
                    <td className="py-5"><div className="h-4 w-52 bg-zinc-100/80 rounded" /></td>
                    <td className="py-5 text-center"><div className="h-4 w-12 bg-zinc-100/40 mx-auto rounded" /></td>
                    <td className="py-5 text-center"><div className="h-4 w-12 bg-zinc-100/40 mx-auto rounded" /></td>
                    <td className="py-5 text-center"><div className="h-4 w-12 bg-zinc-100/40 mx-auto rounded" /></td>
                    <td className="py-5 text-center"><div className="h-4 w-12 bg-zinc-100/40 mx-auto rounded" /></td>
                    <td className="py-5 text-center"><div className="h-4 w-12 bg-zinc-100/40 mx-auto rounded" /></td>
                    <td className="py-5 text-center"><div className="h-4 w-20 bg-zinc-100/80 mx-auto rounded" /></td>
                    <td className="py-5 text-center"><div className="h-4 w-14 bg-zinc-100/80 mx-auto rounded" /></td>
                    <td className="py-5 text-right"><div className="h-4 w-14 bg-zinc-100/40 ml-auto rounded" /></td>
                  </tr>
                ))
                ...
                <tr>
                  <td colSpan={9} className="py-12 text-center text-zinc-500">
                    <div className="flex flex-col items-center justify-center gap-3">
                      ...
                      <p className="font-bold text-zinc-500">No matching courses or digital products found.</p>
                ...
                processedItems.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-zinc-50/40 transition-colors border-zinc-200/60">
                      <td className="py-4">
                        <div className="flex items-center gap-3">
                          <div className={`p-2 rounded-2xl border ${
                            item.type === "Course" 
                              ? "bg-violet-500/5 border-violet-500/10 text-violet-400" 
                              : "bg-brand-500/5 border-zinc-200/60 text-yellow-500"
                          }`}>
                            {item.type === "Course" ? <BookOpen className="w-4 h-4" /> : <Package className="w-4 h-4" />}
                          </div>
                          <div>
                            <p className="font-extrabold text-zinc-900 leading-normal">{item.title}</p>
                      ...
                      <td className="py-4 text-center font-extrabold text-zinc-800">
                        <div className="flex items-center justify-center gap-1.5">
                      ...
                      <td className="py-4 text-center font-extrabold text-yellow-500">
                        <div className="flex items-center justify-center gap-1.5">
                          <MousePointerClick className="w-3.5 h-3.5 text-yellow-500/60" />
                      ...
                      <td className="py-4 text-center font-black text-yellow-500">
