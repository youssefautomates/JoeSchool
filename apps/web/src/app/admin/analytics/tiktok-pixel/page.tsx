...
const formatPrice = (price: number, currency: any) => formatPriceRaw(price, currency).replace("EGP", "L.E");
    ...
    <div className="space-y-8 font-sans text-zinc-900 min-h-screen pb-16">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 pb-6 border-b border-zinc-200/60">
        <div>
          <h1 className="text-2xl font-black tracking-tight text-zinc-900 flex items-center gap-3">
            TikTok Pixel & TikTok Ads Insights
            <Zap className="w-6 h-6 text-yellow-500" />
          </h1>
          <p className="text-zinc-500 text-xs mt-1">
            Monitor and track TikTok Pixel events directly tied to actual database transactions.
          </p>
        </div>

        <div className="flex items-center gap-3">
          <span className={`px-4 py-2 rounded-2xl text-xs font-bold border ${
            stats.completePayment > 0 ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" : "bg-zinc-500/10 text-zinc-500 border-zinc-500/20"
          }`}>
            {stats.status}
          </span>
          <button
      ...
      <div className="p-6 rounded-2xl bg-slate-50 border border-zinc-200/60 shadow-sm border border-zinc-200/60 space-y-4">
        ...
        <p className="text-xs text-zinc-500 font-semibold">
          Select a specific product targeted by TikTok Ads to isolate and view only its associated dynamic Pixel events:
        </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <tbody className="divide-y divide-white/5 text-xs">
                {funnelData.map((item) => (
                  <tr key={item.name} className="hover:bg-zinc-50/40 transition-colors">
                    ...
                    <td className="py-4 text-center font-bold text-yellow-500">{item.events > 0 ? "TikTok Events API + Web Pixel" : "None"}</td>
        ...
        <div className="rounded-2xl bg-slate-50 border border-zinc-200/60 p-6 shadow-sm border border-zinc-200/60 flex flex-col justify-between h-fit">
          <div className="space-y-6">
            <div>
              <h3 className="text-xs font-bold uppercase tracking-wider text-zinc-500">TikTok Attribution & ROAS Analysis</h3>
              <p className="text-xs text-zinc-500 leading-relaxed mt-2 font-semibold">
                ROAS metrics are computed directly based on cumulative transaction value realized for chosen items.
              </p>
            </div>
            
            <div className="space-y-3">
              <div className="flex justify-between items-center p-3 rounded-2xl bg-zinc-50/40 text-xs font-semibold">
                <span className="text-zinc-500">Estimated ROAS Score</span>
                <span className="text-emerald-400 font-bold">{stats.roas}</span>
              </div>
              <div className="flex justify-between items-center p-3 rounded-2xl bg-zinc-50/40 text-xs font-semibold">
      ...
      <div className="rounded-2xl bg-slate-50 border border-zinc-200/60 p-8 shadow-sm border border-zinc-200/60">
        <h3 className="text-xs font-bold uppercase tracking-wider text-zinc-500 mb-8">TikTok Pixel Event Funnel Breakdown</h3>
