...
export function ReviewSelectorPortal({ onSelect }: ReviewSelectorPortalProps) {
  const cards = [
    ...
    },
    {
      ...
      icon: BookOpen,
      color: "from-brand-950/40 via-brand-900/10 to-transparent border-brand-500/15 text-yellow-500 hover:border-zinc-200/60 hover:shadow-[0_0_30px_rgba(29, 78, 216,0.1)]",
      glow: "bg-[#1D4ED8]/5"
    },
    {
  ...
  return (
    <div className="space-y-8 py-4 font-sans" dir="ltr">
      <div className="text-center max-w-2xl mx-auto space-y-3">
        <h1 className="text-3xl md:text-5xl font-black text-zinc-900 leading-tight font-sans">
        ...
        {cards.map((card, idx) => {
          ...
          return (
            <motion.div
              ...
              className={`relative bg-slate-50 border p-8 rounded-3xl cursor-pointer transition-all duration-300 group overflow-hidden flex flex-col justify-between hover:scale-[1.02] shadow-sm border border-zinc-200/60 h-[280px] bg-gradient-to-br ${card.color}`}
            >
              {/* Ambient light glow */}
              ...
              <div className="space-y-4 relative z-10">
                <div className="w-12 h-12 rounded-2xl bg-zinc-100/40 border border-zinc-200 flex items-center justify-center text-current group-hover:bg-zinc-100/80 transition-colors">
                  <Icon className="w-6 h-6" />
                </div>
                <div className="space-y-2">
                  <h3 className="text-lg md:text-xl font-bold text-zinc-900 group-hover:text-yellow-500 transition-colors font-sans">
                    {card.title}
                  </h3>
                  <p className="text-zinc-500 text-[11px] md:text-xs leading-relaxed line-clamp-4 font-sans">
                    {card.desc}
                  </p>
                </div>
              </div>

              <div className="flex items-center gap-1.5 text-[10px] md:text-xs font-bold text-zinc-700 group-hover:text-zinc-900 transition-colors mt-4 self-start font-sans">
