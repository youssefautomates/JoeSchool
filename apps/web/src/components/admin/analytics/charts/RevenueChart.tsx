...
export default function RevenueChart({ data, revenueGrowth, currencySymbol = "L.E" }: RevenueChartProps) {
  if (!data || data.length === 0) {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center border border-dashed border-white/5 rounded-2xl p-6 text-center text-zinc-500 text-xs">
        No transaction logs found in selected window.
      </div>
    );
  }
    ...
    <div className="w-full h-full flex flex-col justify-between font-sans">
      <div className="flex items-center justify-between mb-5 pb-4 border-b border-white/5">
        <div>
          ...
          <p className="text-[10px] text-zinc-500 mt-0.5">Live completed transactional volume in local currency</p>
        </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 10, right: 5, left: -25, bottom: 0 }}>
            ...
            <Tooltip 
              ...
              formatter={(value: any, name: any) => [`${value} ${currencySymbol}`, name]}
            />
            <Area type="monotone" dataKey="Revenue" stroke="#D6004B" strokeWidth={1.5} fillOpacity={1} fill="url(#revenueGlow)" name="Revenue" />
            <Area type="monotone" dataKey="Profit" stroke="#10b981" strokeWidth={1.5} fillOpacity={1} fill="url(#profitGlow)" name="Profit Margin" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </div>
